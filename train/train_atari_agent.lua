...
cmd:option('-save_versions', 125000, '')
...
cmd:option('-source_games', '', 'Neural progressive learner uses these games')
...
while step < opt.steps do
   ...
   if step % opt.save_freq == 0 or step == opt.steps then
      ...
      if opt.save_versions > 0 then			
            prev_file = filename .. "_" .. (math.floor(step / opt.save_versions)-1) .. '.t7'
			local f=io.open(prev_file,"r")
			if f~=nil then 				
				os.remove(prev_file)
